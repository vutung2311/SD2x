...
public class PlagiarismDetector {
    ...
    public static Map<String, Integer> detectPlagiarism(String dirName, int windowSize, int threshold) {
        ...
        Map<String, Set<String>> filePhrasesCache = new HashMap<>();
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            for (int j = i + 1; j < files.length; j++) {
                ...
                Set<String> file1Phrases = getOrCreatePhrases(dirName, windowSize, filePhrasesCache, file1);
                Set<String> file2Phrases = getOrCreatePhrases(dirName, windowSize, filePhrasesCache, file2);
    ...
    private static Set<String> getOrCreatePhrases(String dirName, int windowSize, Map<String, Set<String>> filePhrasesCache, String filename) {
        String key = dirName + "/" + filename + "/" + windowSize;
        if (!filePhrasesCache.containsKey(key)) {
            filePhrasesCache.put(key, createPhrases(dirName + "/" + filename, windowSize));
        }
        return filePhrasesCache.get(key);
    }
